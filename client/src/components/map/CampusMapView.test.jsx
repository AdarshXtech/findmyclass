...
describe('CampusMapView', () => {
  it('shows the real map surface, next class, privacy copy, and manual start', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Central Library' })).toBeVisible()
    expect(screen.getByRole('button', { name: 'Stadium Canteen' })).toBeVisible()
    expect(screen.getByRole('button', { name: 'BBD Girls Hostel' })).toBeVisible()
    expect(screen.getByRole('button', { name: 'Auditorium' })).toBeVisible()
