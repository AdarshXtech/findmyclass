...
function displayFloor(entry) {
  return entry.floorLabel || entry.shortFloor || entry.floor || 'Not listed'
}

export default function LocationHeader({ entry, compact = false, highlighted = false, inline = false }) {
  ...
  const secondaryLocation = [floor, wing].filter(Boolean).join(' \u00b7 ')
  ...
  if (inline) {
    return (
      ...
      >
        <p className="font-display text-xl font-bold leading-tight [overflow-wrap:anywhere]">{title}</p>
        <p className="mt-1 font-mono text-xs font-bold uppercase tracking-wide text-text-secondary [overflow-wrap:anywhere]">{secondaryLocation}</p>
      </div>
    )
  }
