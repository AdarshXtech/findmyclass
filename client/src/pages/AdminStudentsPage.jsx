...
export default function AdminStudentsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!payload.name || !payload.phone || !payload.course || !payload.branch || !payload.year || !payload.section) {
    ...
    <div className="space-y-6">
      ...
      <section className="glass-card rounded-2xl p-6">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input className="input-field" placeholder="Course (e.g. B.Tech)" value={form.course} onChange={(e) => setForm({ ...form, course: e.target.value })} />
          <input className="input-field" placeholder="Branch (e.g. CSE)" value={form.branch} onChange={(e) => setForm({ ...form, branch: e.target.value })} />
          <input className="input-field" type="number" min={1} max={8} placeholder="Year" value={form.year} onChange={(e) => setForm({ ...form, year: e.target.value })} />
          <input className="input-field" placeholder="Section (e.g. CSE-A)" value={form.section} onChange={(e) => setForm({ ...form, section: e.target.value.toUpperCase() })} />
