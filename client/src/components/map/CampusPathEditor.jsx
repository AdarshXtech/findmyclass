...
export default function CampusPathEditor() {
  const [nodes, setNodes] = useState(() => CAMPUS_PATH_NODES.map((node) => ({ ...node })))
  const [edges, setEdges] = useState(() => CAMPUS_PATH_EDGES.map((edge) => [...edge]))
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={download} className="min-h-11 bg-result-slate px-4 py-2 font-bold text-text-on-dark">Export campusPaths.js</button>
      </div>

      <p className="text-sm font-semibold text-text-secondary" role="status">
        {activeNodeId ? `Tracing from ${activeNodeId}` : 'Ready for a new path'}
      </p>
